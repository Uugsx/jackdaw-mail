...
export class GraphFolder extends Folder {
  ...
  async markAllRead(): Promise<void> {
    ...
    // Graph has no folder-level mark-all-read; update loaded messages one by one.
    await Promise.allSettled(this.messages.contents.map(msg => msg.markRead(true)));
  }

  async markAllUnread(): Promise<void> {
    await super.markAllUnread();
    await Promise.allSettled(this.messages.contents.map(msg => msg.markRead(false)));
  }
